...
Candidate-triggered execution of Java code during assessment taking. The backend proxies runs to a self-hosted Piston sandbox engine — the browser never calls the engine directly — with a defined request/response contract, outcome classification, serialized rate limiting, and failure handling, plus the candidate-facing Run panel UI.

...
- **THEN** all flags (FLAGGED, UNDER_REVIEW, RESOLVED, DISMISSED) are loaded and displayed ordered by most recent first
...
The flagged submissions page SHALL include a status filter dropdown with options: All, Flagged, Under Review, Resolved, Dismissed. The default selection SHALL be "All" (showing every status). Selecting a status SHALL filter the visible rows to that status only.

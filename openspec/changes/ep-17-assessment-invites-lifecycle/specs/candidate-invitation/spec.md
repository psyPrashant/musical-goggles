...
The system SHALL construct the assessment access link in invitation emails using the value of `app.frontend.base-url` configuration property, not the backend server address derived from the HTTP request.

#### Scenario: Invite email link uses frontend base URL
- **WHEN** a staff member sends an assessment invitation
- **THEN** the access link in the invitation email is constructed as `{app.frontend.base-url}/assessment/{assessmentId}/take?token={token}`

#### Scenario: Frontend base URL is configurable
- **WHEN** `app.frontend.base-url` is set to `https://app.example.com` in application configuration

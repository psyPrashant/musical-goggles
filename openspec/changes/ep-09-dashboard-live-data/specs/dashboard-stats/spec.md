...
- **THEN** the system returns HTTP 200 with:
  ...
  - `pipeline`: object with `invited`, `inProgress`, `pendingReview`, `completed` integer counts
...
The dashboard component SHALL replace all hardcoded values with live data from `GET /api/dashboard/stats`. A loading state SHALL be shown while the request is in flight.
...
- **AND** the pipeline bar reflects actual stage counts

...
The system SHALL expose a `GET /api/submissions/{submissionId}/result` endpoint accessible to `ROLE_RECRUITER` and `ROLE_ADMIN`. The response SHALL aggregate all `AnswerScore` values for the submission into a total score, list each question with its answer, score, and feedback, indicate the marking completeness status (`FULLY_MARKED` or `PENDING_REVIEW`), and include submission metadata (candidate name, assessment title, submitted at). The `maxScore` field in the response SHALL represent `SUM(question.maxScore)` across all questions in the assessment — the weighted maximum possible score — not a raw question count.
...
#### Scenario: maxScore reflects weighted question values
- **WHEN** an assessment has questions worth 1, 2, and 5 points
- **THEN** the result summary `maxScore` is 8 (not 3)

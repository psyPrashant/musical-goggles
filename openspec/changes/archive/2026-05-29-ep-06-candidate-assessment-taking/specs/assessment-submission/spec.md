...
The system SHALL expose a `POST /api/take/submit` endpoint secured by the candidate session JWT. The endpoint SHALL lock the `CandidateSubmission` by setting its status to `SUBMITTED` (or `AUTO_SUBMITTED` when the `autoSubmitted` flag is true), record `submittedAt`, and transition the associated `CandidateInvitation` status to `COMPLETED` — all within a single transaction. The response SHALL include the submission ID, final status, and `submittedAt` timestamp.
...
- **THEN** the response is HTTP 200 with the existing submission status and original `submittedAt` — no duplicate records created
